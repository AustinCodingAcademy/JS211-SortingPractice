...
let numbersArray = strNums.map(stringsToNumbs)
// console.log(`This is the array of numbers: ${numbersArray}`)

// (2)  With the same numbers, find the sum of the even values
let simpleArray = [1, 2, 3, 4, 5, 6,]
let sum = 0
const addEvens = (array) => {
    for (i=0; i< array.length; i++) {
        // console.log(`AE this is current array[i]: ${array[i]}`)
        if (array[i]%2 == 0){
...
console.log(addEvens(simpleArray))

const sumEvens = addEvens(numbersArray)
console.log(`Sum of all the evens: ${sumEvens}`)
console.log(sumEvens);

// (3) Find the index of the first value when added to is index = 512 (#ATX!!)
const nums = numbersArray
function find512(num){
...
console.log(`index: ${atxIdx}, value: ${nums[atxIdx]}`);
  ...
  let test = weather[1]["weather_state_name"]
  console.log(test)
  function findWeatherStatesNames(x) {
      return x["weather_state_name"] //how do I get this to be unique?
  }
  const weatherStates = weather.map(findWeatherStatesNames)  //this is an array. I can filter this array to find only unique elements
  console.log(weatherStates)

function findUniqueWSN (element, index, array){
  return array.indexOf(element) === index
}

const uniqueWeatherStates = weatherStates.filter(findUniqueWSN)
...
// (5) find the id of the object in weather that has a min_temp of 15.915
  ...
  function idAtTemp(array) {
    for (let i = 0; i < array.length; i++) {
        if (array[i]["min_temp"] == 15.915) {
          // console.log(array[i]["min_temp"])
          // console.log(array[i]["id"])
          return array[i]["id"]
      }
    }
  }
  let idealTemp = idAtTemp(weather)
  console.log(idealTemp)
  let foundID = null
  function idAtTemp(element, index, array){
  if (element["min_temp"] == 15.915) {
    console.log(element["min_temp"])
  //   console.log(element["id"])
    foundID = element["id"]
  }
}

 weather.forEach(idAtTemp)
  console.log(foundID);
