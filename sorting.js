...
// can convert nums using map method
...
// if element is not contained within output array, push that element to the output array
const weatherStates = (arr) => {
  const outputArr = [];
  arr.forEach(function (element) {
    if (!outputArr.includes(element.weather_state_name)) {
      outputArr.push(element.weather_state_name);
    }
  });
  return outputArr;
};
...
// I used a higher order function via forEach method
// Correctly returns ['Light Cloud', 'Heavy Cloud', 'Showers']

//find the id of the object in weather that has a min_temp of 15.915

let idealTemp = (arr) => {
  const [index] = arr.filter((element) => element.min_temp === 15.915);
  return index.id;
};
console.log(idealTemp(weather)); // 6696130918219776
